...
def inverse(matrix):
    ...
    # If determinant is 0, matrix is singular (not invertible)
    if det == 0:
        return None
    ...
    for i in range(n):
        adjugate_row = []
        for j in range(n):
            adjugate_row.append(cofactor_matrix[j][i])  # Transpose: swap i and j
        adjugate_matrix.append(adjugate_row)
    
    # Calculate inverse matrix: (1/det) * adjugate
    inverse_matrix = []
    for i in range(n):
        inverse_row = []
        for j in range(n):
